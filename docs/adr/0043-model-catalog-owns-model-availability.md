...
在 `packages/backend/src/workspace/model-catalog/` 建立 **Model Catalog** module，它是后端唯一拥有 Model 可用性的地方。它持有唯一的 Pace `ModelRuntime`（由组合根创建，同一实例注入 provider auth），统一映射 capability，决定凭证变化后的刷新策略，并把刷新扇出到运行中的 Session。Settings、Session Draft composer、Live Session View 都只读它。
